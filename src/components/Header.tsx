...
function Stat({ label, value, icon: Icon, accent }: { label: string; value: string | number; icon: LucideIcon; accent: string }) {
  return (
    <div className="flex items-center gap-3 rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-3 transition-all hover:border-slate-700 hover:bg-slate-800/60">
      <span className={`flex h-9 w-9 items-center justify-center rounded-lg bg-slate-800 ${accent}`}>
        <Icon size={16} />
      </span>
      <div>
...
export default function Header({ bundle }: { bundle: CourseBundle }) {
  const { course } = bundle
  return (
    <header className="relative overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-900 via-slate-900 to-slate-900 p-8 shadow-xl md:p-10">
      ...
      <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="max-w-xl">
          <div className={`mb-4 inline-flex items-center gap-2 rounded-full border bg-slate-950/50 px-3 py-1 text-xs font-medium ${bundle.accent.border} ${bundle.accent.text}`}>
